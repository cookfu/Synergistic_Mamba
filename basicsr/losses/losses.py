...
class ColorLoss(nn.Module):
    ...
    def forward(self, predict, target, weight=None, **kwargs):
        ...
        return color_loss*self.loss_weight
...
@LOSS_REGISTRY.register()
class MutiVGGLoss(nn.Module):
    def __init__(self, loss_weight=1.0, reduction='mean',muti_scale=[1,0.8,0.4]):
        super(MutiVGGLoss, self).__init__()
        self.loss_weight = loss_weight
    ...
    def forward(self, out, out_2 , out_4, target):
        out_vgg, gt_vgg = self.vgg(out), self.vgg(target)
        target_2 = F.interpolate(target, scale_factor=0.5, mode='bilinear', align_corners=True)
        target_4 = F.interpolate(target, scale_factor=0.25, mode='bilinear', align_corners=True)
        # print("out_2.shape  ",out_2.shape)
        # print("target_2.shape  ",target_2.shape)
        out_vgg_2, gt_vgg_2 = self.vgg(out_2), self.vgg(target_2)
        out_vgg_4, gt_vgg_4 = self.vgg(out_4), self.vgg(target_4)
        loss = 0
        for i in range(len(out_vgg)):
            # print(x_vgg[i].shape, y_vgg[i].shape)
            loss += self.weights[i] * self.criterion(out_vgg[i], gt_vgg[i].detach())*self.muti_scale[0]
            loss += self.weights[i] * self.criterion(out_vgg_2[i], gt_vgg_2[i].detach())*self.muti_scale[1]
            loss += self.weights[i] * self.criterion(out_vgg_4[i], gt_vgg_4[i].detach())*self.muti_scale[2]
...
class SSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        return (1.0 - _ssim(img1, img2, window, self.window_size, channel, self.size_average))*self.loss_weight