...
def train_pipeline(root_path): #训练的主要流程函数
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            # 保存模型和训练状态
            if current_iter % opt['logger']['save_checkpoint_freq'] == 0 and current_iter > opt['val']['start_val_iter']:
                logger.info('Saving models and training states.')
                model.save(epoch, current_iter)

            if current_iter % opt['logger']['save_latest_freq'] == 0 and current_iter > opt['val']['start_val_iter']:
                logger.info('Saving models and training states.')
                model.save(epoch, current_iter)

            # 进行模型验证
            if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0) and current_iter > opt['val']['start_val_iter']:
